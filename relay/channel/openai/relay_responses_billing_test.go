...
// Fork deviation: upstream coerces response.created_at to an integer only for
// SGLang (rewriteSGLangResponsesCreatedAt) and relays other channels' timestamps
// verbatim. This fork coerces unconditionally (patchResponsesUsageDoc in
// relay/channel/openai/relay_responses.go) so strict Responses clients such as
// Codex never receive a float timestamp.
func TestOaiResponsesStreamHandlerCoercesNonSGLangCreatedAt(t *testing.T) {
	gin.SetMode(gin.TestMode)
	oldTimeout := constant.StreamingTimeout
	constant.StreamingTimeout = 30
	t.Cleanup(func() {

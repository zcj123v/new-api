...
func ApplyResponsesUsage(dst *dto.Usage, src *dto.Usage) {
	...
	// Fork deviation: some upstreams (e.g. Moonshot/Kimi) report reasoning
	// tokens at the top level of usage rather than inside
	// completion_tokens_details, which NormalizeResponsesUsage does not read.
	reasoningTokens := src.ReasoningTokens
	if reasoningTokens == 0 && src.OutputTokensDetails != nil {
		reasoningTokens = src.OutputTokensDetails.ReasoningTokens
	}
	if reasoningTokens != 0 {
		incoming.CompletionTokenDetails.ReasoningTokens = reasoningTokens
	}
	incoming.PromptCacheHitTokens = src.PromptCacheHitTokens
	dto.MergeUsageNonZero(dst, incoming)
	if reasoningTokens != 0 {
		dst.ReasoningTokens = reasoningTokens
	}

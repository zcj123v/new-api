...
func TestDecideRelayRetryReasons(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		// Fork deviation: 504 is retryable on this fork (see
		// setting/operation_setting/status_code_ranges.go), so 524 is the only
		// remaining always-skipped status. See the 504 case below.
		{name: "always skipped status", err: upstream(524), retries: 1, want: PolicyDecision{Action: "stop", Reason: "system_retry_exclusion", Source: "system"}},
		// Fork deviation: upstream excludes 504 from retries; this fork retries it
		// because our upstreams answer 504 with a bare "stream timeout" body after
		// their own proxy timeout, and giving up costs the client a five-minute
		// stall with no failover.
		{name: "504 stays retryable (fork deviation)", err: upstream(http.StatusGatewayTimeout), retries: 1, want: PolicyDecision{Action: "retry", Reason: "retry_status_matched", Source: "global"}},

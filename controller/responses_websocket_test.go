...
func TestResponsesWebSocketDialsNativeResponsesChannelTypes(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// Fork deviation: the relayed response.completed is re-serialized to
			// inject usage.output_tokens_details (strict Responses clients such as
			// Codex require the field), which also normalizes key order. See
			// relay/channel/openai/relay_responses.go.
			assert.Contains(t, string(body), `{"response":{"id":"first","status":"completed","usage":{"input_tokens":1000,"output_tokens":10,"output_tokens_details":{"reasoning_tokens":0},"total_tokens":1010}},"type":"response.completed"}`)
			select {
			case <-fixture.httpDone:
			case <-time.After(3 * time.Second):
...
func TestResponsesStreamOutcomesPreserveAccounting(t *testing.T) {
	for _, transport := range []string{"websocket", "http-sse"} {
		for _, tc := range []struct {
			// wantRelayed overrides the expected relayed payload when this fork
			// rewrites the terminal event before sending it (see the fork-deviation
			// note on response.completed below).
			name, expression, terminal, wantRelayed string
			...
			// Codex require the field), which also normalizes key order. See
			// relay/channel/openai/relay_responses.go.
			{name: "completed-zero-fixed", expression: `tier("request", fixed(0.002))`, terminal: `{"type":"response.completed","response":{"id":"first","status":"completed","usage":{"input_tokens":0,"output_tokens":0,"total_tokens":0}}}`, wantRelayed: `{"response":{"id":"first","status":"completed","usage":{"input_tokens":0,"output_tokens":0,"output_tokens_details":{"reasoning_tokens":0},"total_tokens":0}},"type":"response.completed"}`},
		} {
